{{ config (
    alias = target.database + '_facebook_performance_by_ad_region_consolidated'
)}}
...
{%- set stg_fields = adapter.get_columns_in_relation(ref('_stg_facebook_ads_insights_region'))
                    |map(attribute="name")
...
WITH 
    ...
    (SELECT 
        ...
        {%- endfor %}
    FROM {{ ref('_stg_facebook_ads_insights_region') }}
    {%- if var('currency') != 'USD' %}
...
{%- set dimensions = ['account_id','campaign_id','adset_id','ad_id','region','attribution_setting'] -%}
{%- set measures = adapter.get_columns_in_relation(ref('facebook_ads_insights_region'))
                    |map(attribute="name")
                    |reject("in",exclude_fields)
