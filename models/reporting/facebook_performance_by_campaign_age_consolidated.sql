{{ config (
    alias = target.database + '_facebook_performance_by_campaign_age_consolidated'
)}}
...
{%- set stg_fields = adapter.get_columns_in_relation(ref('_stg_facebook_campaigns_insights_age'))
                    |map(attribute="name")
...
WITH 
    ...
    (SELECT 
        ...
        {%- endfor %}
    FROM {{ ref('_stg_facebook_campaigns_insights_age') }}
    {%- if var('currency') != 'USD' %}
